defmodule Solution do
  ...
  def find_twenty_picosecond_for_point({x0, y0}, maze, distances) do
    maze
    |> Enum.reduce(MapSet.new(), fn {x, y}, acc ->
      len = abs(x0 - x) + abs(y0 - y)
    ...
    |> Enum.filter(fn {p1, p2, _len} -> distances[p1] < distances[p2] end)
